...
use Clover\Routing\Command;
use Clover\Routing\RouterException;
use Clover\Routing\Request;
use Closure;
use Exception;
...
class Router
{
    ...
    public function __call($method, $params)
    {
        ...
        if (!in_array(strtoupper($method), explode('|', Request::$validMethods))) {
        ...
        if (in_array($uri, $routes)) {
            ...
            foreach ($this->routes as $data) {
                ...
                if (preg_match('#^' . $route . '$#', $uri, $matched)) {
                    if (Request::validMethod($data['method'], $method)) {
                        $foundRoute = true;
        ...
        if ($classMethods) {
            foreach ($classMethods as $methodName) {
                if (!strstr($methodName, '__')) {
                    $method = 'any';
                    foreach (explode('|', Request::$validMethods) as $m) {
    ...
    public function runRouteMiddleware($middleware, $type)
    {
        if ($type === 'before') {
            if (!is_null($middleware['group'])) {
                $this->Command()->beforeAfter($middleware['group'][$type]);
            }
            $this->Command()->beforeAfter($middleware[$type]);
        } else {
            $this->Command()->beforeAfter($middleware[$type]);
            if (!is_null($middleware['group'])) {
                $this->Command()->beforeAfter($middleware['group'][$type]);
    ...
    {
        return Command::getInstance($this->baseFolder, $this->paths, $this->namespaces);
    }
    ...
    {
        $this->Command()->runRoute($command, $params);
    }
